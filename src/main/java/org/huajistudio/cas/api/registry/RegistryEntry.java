...
import org.huajistudio.cas.client.CellularAutomatonSimulator;
...
public interface RegistryEntry<V> {
    ...
    class Impl<T extends RegistryEntry<T>> implements RegistryEntry<T> {
        ...
        public final T setRegistryName(String name) {
            ...
            try {
                plugin = CellularAutomatonSimulator.getPluginManager().whichPlugin(Class.forName(Thread.currentThread().getStackTrace()[1].getClassName()));
            } catch (ClassNotFoundException ignored) {
            }
            String prefix = plugin == null ? "opensimcity" : plugin.getPluginId().toLowerCase();
            if (!oldPrefix.equals(prefix) && oldPrefix.length() > 0) {
                SyncLogger.getLogger().warn("Dangerous alternative prefix `{}` for name `{}`, expected `{}` invalid registry invocation/invalid name?", oldPrefix, name, prefix);
                prefix = oldPrefix;
            }
